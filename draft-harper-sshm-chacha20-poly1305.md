...
Poly1305 requires a 32-byte one-time key per **RFC 7539**, consisting of 16 bytes **r** (with specific bit clamping) followed by 16 bytes **s**. The one-time Poly1305 key used to authenticate packets during a keying interval **MUST** be derived using **K\_E**, the **Fixed\_Nonce\_Prefix**, and a zero **Block\_Counter**. The derivation steps involve constructing a 12-byte **Nonce\_Derivation = Fixed\_Nonce\_Prefix // 0x00000000**, initializing ChaCha20 with **Key = K\_E** and **Nonce = Nonce\_Derivation** with an internal block counter set to 0, generating the first 64 bytes of the ChaCha20 stream (or at minimum the first 32 bytes required), taking the first 32 bytes as the Poly1305 key **K\_poly**, and applying Poly1305 **r** masking as per RFC 7539. Implementations **MUST** ensure the ChaCha20 stream used for Poly1305 key derivation uses the same key **K\_E** and the same **Fixed\_Nonce\_Prefix** as used for packet encryption, with **Block\_Counter** set to zero for this derivation. The use of a zero block counter ensures the Poly1305 key is unique per keying interval and distinct from per-packet block counters used for encryption.

## K\_MAC handling

**K\_MAC** (16 bytes) is **NOT** used directly as the final Poly1305 key, but is reserved as a seed that **MAY** be incorporated into the ChaCha20 initial block for additional domain separation, only if this procedure is deterministic, documented, and ensures the resulting Poly1305 key is unique per keying interval and per direction. The mandatory minimum is that **K\_E** and **Fixed\_Nonce\_Prefix** are used as described to produce the Poly1305 key; any additional use of **K\_MAC** **MUST NOT** violate ChaCha20 or Poly1305 assumptions and **MUST** be clearly specified. Implementations **MUST** store the derived Poly1305 key (**r//s**) with appropriate protections and **MUST** discard the ChaCha20 keystream material after use.
