...
The ChaCha20/Poly1305 construction for SSH transport layer authenticated encryption is detailed, focusing on separating a fixed, session-unique nonce prefix from the per-packet block counter to mitigate the risks of nonce reuse stemming from sequence number manipulation or errors. This specification adheres to **RFC 7539** and defines key material layout, nonce construction, MAC key generation, packet processing, and rekeying limits.
...
This specification uses the key words **MUST**, **MUST NOT**, **REQUIRED**, **SHOULD**, **SHOULD NOT**, **RECOMMENDED**, and **MAY** as defined in RFC 2119. **Fixed\_Nonce\_Prefix** is a 96-bit (12-byte) constant value derived from the key exchange outputs for the life of a given SSH encryption key, produced by a KDF using the outputs of the SSH Key Exchange (KEX) and must be unique per direction (client→server and server→client) and per keying interval. **Block\_Counter** is a 32-bit unsigned integer used as ChaCha20's block counter, derived from the 32-bit packet sequence number for a packet as defined in Section 6. **Nonce** is the 96-bit ChaCha20/Poly1305 nonce formed by concatenating Fixed\_Nonce\_Prefix || Block\_Counter, expressed in network byte order for ChaCha20 input. **K\_E** is the 32-byte (256-bit) ChaCha20 encryption key. **K\_MAC** is a 16-byte seed value used to derive the Poly1305 one-time key stream ($\mathbf{r}$ and $\mathbf{s}$) for the authentication tag; the final Poly1305 key is 32 bytes ($\mathbf{r||\mathbf{s}}$) as specified in RFC 7539 after bit masking of $\mathbf{r}$. A **Keying Interval** is the lifetime of a specific set of K\_E and K\_MAC values, typically from a KEX until a rekey event.
